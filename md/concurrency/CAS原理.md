...
- [01、CAS(Compare-And-Swap)简介](#01cascompare-and-swap简介)
    - [1、CAS 的思路(cpu指令保证原子性)](#1cas-的思路cpu指令保证原子性)
    - [2、CAS 的语义](#2cas-的语义)
- [02、CAS 和乐观锁的关系，什么时候会用到 CAS？](#02cas-和乐观锁的关系什么时候会用到-cas)
    - [1、ConcurrentHashMap](#1concurrenthashmap)
    - [2、ConcurrentLinkedQueue](#2concurrentlinkedqueue)
    - [3、数据库](#3数据库)
    - [4、原子类](#4原子类)
- [03、CAS 有什么缺点？（ ABA 问题、自旋时间过长以及线程安全的范围不能灵活控制）](#03cas-有什么缺点-aba-问题自旋时间过长以及线程安全的范围不能灵活控制)
    - [1、ABA 问题](#1aba-问题)
    - [2、自旋时间过长](#2自旋时间过长)
    - [3、范围不能灵活控制](#3范围不能灵活控制)

<!-- /TOC -->

# 01、CAS(Compare-And-Swap)简介
...
```java
public class SimulatedCAS {//模拟CAS操作，等价代码
...
方法“casTabAt”，这个方法名就带有 “CAS”，可以猜测它一定是和 CAS 密不可分了，下面给出 casTabAt 方法的代码实现：
...
可以看出，在 offer 方法中，有一个 for 循环，这是一个死循环，CAS 相关的方法是 casNext 方法，用于更新节点。那么如果执行 p 的 casNext 方法失败的话，casNext 会返回 false，那么显然代码会继续在 for 循环中进行下一次的尝试。所以在这里也可以很明显的看出 ConcurrentLinkedQueue 的 offer 方法使用到了 CAS。
...
在原子类中，例如 AtomicInteger，也使用了 CAS，也就是 AtomicInteger 的 getAndAdd 方法，该方法代码如下所示：
...
- 最后一个参数 newValue 是希望修改为的新值 ，等于之前取到的数值 var5 再加上 var4，而 var4 就是我们之前所传入的 delta，delta 就是我们希望原子类所改变的数值，比如可以传入 +1，也可以传入 -1。
...
# 03、CAS 有什么缺点？（ ABA 问题、自旋时间过长以及线程安全的范围不能灵活控制）
