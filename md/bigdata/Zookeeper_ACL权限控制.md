...
- [Zookeeper ACL](#zookeeper-acl)
    - [一、前言](#一前言)
    - [二、使用Shell进行权限管理](#二使用shell进行权限管理)
        - [2.1 设置与查看权限](#21-设置与查看权限)
        - [2.2 权限组成](#22-权限组成)
        - [2.3 添加认证信息](#23-添加认证信息)
        - [2.4 权限设置示例](#24-权限设置示例)
            - [1. world模式](#1-world模式)
            - [2. auth模式](#2-auth模式)
            - [3. digest模式](#3-digest模式)
            - [4. ip模式](#4-ip模式)
            - [5. super模式](#5-super模式)
    - [三、使用Java客户端进行权限管理](#三使用java客户端进行权限管理)
        - [3.1 主要依赖](#31-主要依赖)
        - [3.2 权限管理API](#32-权限管理api)
