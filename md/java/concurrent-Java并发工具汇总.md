...
- [2、JUC并发工具](#2juc并发工具)
    ...
    - [4、阻塞队列](#4阻塞队列)
    - [5、原子类](#5原子类)
    - [6、ThreadLocal](#6threadlocal)
    - [7、Future](#7future)
    - [8、线程协作](#8线程协作)
- [3、深入浅出底层原理，知其所以然](#3深入浅出底层原理知其所以然)
    - [1、Java 内存模型](#1java-内存模型)
    - [2、CAS 原理](#2cas-原理)
    - [3、死锁问题](#3死锁问题)
    - [4、final 关键字和“不变性”](#4final-关键字和不变性)
    - [5、AQS 框架](#5aqs-框架)

<!-- /TOC -->

备注：复习java并发知识可以按照该列表进行，会不断的补充完善
...
在线程池部分中我们首先给出了 3 点使用线程池的原因，也就是说，使用线程池比手动创建线程好的地方在于：

- 1、可以解决线程生命周期的系统开销问题，同时还可以加快响应速度；
- 2、可以统筹内存和 CPU 的使用，避免资源使用不当；
- 3、可以统一管理资源。

在了解了线程池的好处之后，就需要掌握线程池的各个参数的含义，即 corePoolSize、maxPoolSize、keepAliveTime、workQueue、ThreadFactory、Handler，我们需要知道每个参数代表什么含义。
