...
- [1、Buffer](#1buffer)
    - [1、position(当前位置)、limit、capacity](#1position当前位置limitcapacity)
    - [2、初始化 Buffer](#2初始化-buffer)
    - [3、填充 Buffer](#3填充-buffer)
    - [4、提取 Buffer 中的值](#4提取-buffer-中的值)
    - [5、mark() & reset()实现重复读](#5mark--reset实现重复读)
    - [6、rewind() & clear() & compact()](#6rewind--clear--compact)
- [2、Channel](#2channel)
    - [1、FileChannel](#1filechannel)
    - [2、SocketChannel](#2socketchannel)
    - [3、ServerSocketChannel](#3serversocketchannel)
    - [4、DatagramChannel](#4datagramchannel)
- [3、Selector](#3selector)
    - [1、select()](#1select)
    - [2、selectNow()](#2selectnow)
    - [3、select(long timeout)](#3selectlong-timeout)
    - [4、wakeup()](#4wakeup)
- [参考](#参考)
...
介绍 Java NIO 中三大组件 Buffer、Channel、Selector 的使用。

# 1、Buffer

一个 Buffer 本质上是内存中的一块，我们可以将数据写入这块内存，之后从这块内存获取数据。

java.nio 定义了以下几个 Buffer 的实现，如下图所示：

![](../../pic/2021-03-28/2021-03-28-12-49-56.png)

其实核心是最后的` ByteBuffer`，前面的一大串类只是包装了一下它而已，我们使用最多的通常也是 ByteBuffer。

我们应该将 Buffer 理解为一个数组，IntBuffer、CharBuffer、DoubleBuffer 等分别对应 int[]、char[]、double[] 等。

`MappedByteBuffer用于实现内存映射文件`，也不是本文关注的重点。

我觉得操作 Buffer 和操作数组、类集差不多，只不过大部分时候我们都把它放到了 NIO 的场景里面来使用而已。下面介绍 Buffer 中的几个重要属性和几个重要方法。

## 1、position(当前位置)、limit、capacity

就像数组有数组容量，每次访问元素要指定下标，Buffer 中也有几个重要属性：position、limit、capacity。

![](../../pic/2021-03-28/2021-03-28-12-51-52.png)
...
![](../../pic/2021-03-28/2021-03-28-12-53-46.png)
...
![](../../pic/2021-03-28/2021-03-28-13-03-15.png)
...
![](../../pic/2021-03-28/2021-03-28-13-07-04.png)
...
我们可以`同时监听一个 Channel 中的发生的多个事件`，比如我们要监听 ACCEPT 和 READ 事件，那么指定参数为二进制的 00010001 即十进制数值 17 即可。
