...
- [1、Eureka 服务发现慢的原因](#1eureka-服务发现慢的原因)
    - [1、服务端缓存](#1服务端缓存)
    - [2、客户端缓存](#2客户端缓存)
        - [1、Eureka Client 缓存](#1eureka-client-缓存)
        - [2、Ribbon 缓存](#2ribbon-缓存)
- [2、Spring Cloud 各组件超时](#2spring-cloud-各组件超时)
    - [1、Zuul](#1zuul)
    - [2、Ribbon](#2ribbon)
    - [3、Hystrix](#3hystrix)
    - [4、Feign](#4feign)
- [3、Spring Cloud 各组件重试](#3spring-cloud-各组件重试)
- [4、Spring Cloud 各组件调优](#4spring-cloud-各组件调优)
    - [1、Zuul](#1zuul-1)
    - [2、Feign](#2feign)
    - [3、Zuul动态路由](#3zuul动态路由)
    - [4、Feign 动态日志级别](#4feign-动态日志级别)
...
![](../../pic/2020-11-08/2020-11-08-22-56-11.png)
...
第一层缓存是 readOnlyCacheMap，readOnlyCacheMap 是采用 ConcurrentHashMap来存储数据的，主要负责定时与 readWriteCacheMap 进行数据同步，默认同步时间为 30 秒一次。
...
如果采用的是服务发现方式，就可以通过服务名去进行转发，需要配置 Ribbon 的超时。Ribbon 的超时可以配置全局的 ribbon.ReadTimeout 和 ribbon.ConnectTimeout。也可以在前面指定服务名，为每个服务单独配置，比如 user-service.ribbon.ReadTimeout。
