...
- [1、Hadoop的局限](#1hadoop的局限)
- [2、HBase简介](#2hbase简介)
- [3、HBase Table](#3hbase-table)
- [4、Phoenix](#4phoenix)
- [参考资料](#参考资料)
...
![](../../pic/2021-03-21/2021-03-21-21-59-13.png)
...
但是 `Hadoop 的缺陷在于它只能执行批处理，并且只能以顺序方式访问数据`，这意味着即使是最简单的工作，也必须搜索整个数据集，无法实现对`数据的随机访问`。实现数据的随机访问是传统的关系型数据库所擅长的，但它们却不能用于海量数据的存储。在这种情况下，必须有一种新的方案来解决海量数据存储和随机访问的问题，HBase 就是其中之一 (HBase，Cassandra，couchDB，Dynamo 和 MongoDB 都能存储海量数据并支持随机访问)。
...
![](../../pic/2021-03-21/2021-03-21-22-02-05.png)
