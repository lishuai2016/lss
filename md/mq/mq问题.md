...
- [01、为什么使用消息队列啊？(其实就是优点：解耦、异步、削峰)](#01为什么使用消息队列啊其实就是优点解耦异步削峰)
- [02、消息队列的优缺点？](#02消息队列的优缺点)
- [03、kafka、activemq、rabbitmq、rocketmq都有什么优点和缺点啊？](#03kafkaactivemqrabbitmqrocketmq都有什么优点和缺点啊)
    - [1、ActiveMQ](#1activemq)
    - [2、RabbitMQ](#2rabbitmq)
    - [3、RocketMQ](#3rocketmq)
    - [4、Kafka](#4kafka)
- [04、引入消息队列之后该如何保证其高可用性？(mq服务器自身数据存储机制)](#04引入消息队列之后该如何保证其高可用性mq服务器自身数据存储机制)
    - [（1）RabbitMQ的高可用性](#1rabbitmq的高可用性)
    - [（2）kafka的高可用性](#2kafka的高可用性)
    - [（3）rocketmq的高可用（todo）](#3rocketmq的高可用todo)
- [05、如何保证消息不被重复消费啊（如何保证消息消费时的幂等性）？[幂等]](#05如何保证消息不被重复消费啊如何保证消息消费时的幂等性幂等)
- [06、如何保证消息的可靠性传输（如何处理消息丢失的问题）？](#06如何保证消息的可靠性传输如何处理消息丢失的问题)
    - [（1）rabbitmq](#1rabbitmq)
    - [（2）kafka](#2kafka)
        - [1）消费端弄丢了数据](#1消费端弄丢了数据)
        - [2）kafka弄丢了数据](#2kafka弄丢了数据)
        - [3）生产者会不会弄丢数据](#3生产者会不会弄丢数据)
- [07、如何保证消息的顺序性？](#07如何保证消息的顺序性)
- [08、如何解决消息队列的延时以及过期失效问题？消息队列满了以后该怎么处理？有几百万消息持续积压几小时，说说怎么解决？](#08如何解决消息队列的延时以及过期失效问题消息队列满了以后该怎么处理有几百万消息持续积压几小时说说怎么解决)
    - [（1）大量消息在mq里积压了几个小时了还没解决](#1大量消息在mq里积压了几个小时了还没解决)
- [09、如果让你写一个消息队列，该如何进行架构设计啊？说一下你的思路](#09如果让你写一个消息队列该如何进行架构设计啊说一下你的思路)
- [99、总结一下消息队列相关问题的面试技巧](#99总结一下消息队列相关问题的面试技巧)
...
![mq-解耦前](../../pic/2020-10-17/2020-10-17-22-54-51.png)

![mq-解耦](../../pic/2020-10-17/2020-10-17-22-55-38.png)
...
![mq-异步前](../../pic/2020-10-17/2020-10-17-22-57-19.png)

![异步后](../../pic/2020-10-17/2020-10-17-22-58-55.png)


削峰：每天0点到11点，A系统风平浪静，每秒并发请求数量就100个。结果每次一到11点~1点，每秒并发请求数量突然会暴增到1万条。但是系统最大的处理能力就只能是每秒钟处理1000个请求啊。。。尴尬了，系统会死。。。

![mq-削峰前](../../pic/2020-10-17/2020-10-17-22-59-58.png)

![削峰](../../pic/2020-10-17/2020-10-17-23-00-42.png)
...
# 03、kafka、activemq、rabbitmq、rocketmq都有什么优点和缺点啊？
...
# 04、引入消息队列之后该如何保证其高可用性？(mq服务器自身数据存储机制)
...
![rabbitmq普通集群模式](../../pic/2020-10-17/2020-10-17-23-14-50.png)
...
![rabbitmq镜像集群模式](../../pic/2020-10-17/2020-10-17-23-16-38.png)
...
![](../../pic/2020-10-17/2020-10-17-23-19-40.png)
...
# 05、如何保证消息不被重复消费啊（如何保证消息消费时的幂等性）？[幂等]

能不能避免重复消费？或者重复消费了也别造成系统异常可以吗？这个是MQ领域的基本问题，其实本质上还是问你使用消息队列如何保证幂等性，这个是你架构里要考虑的一个问题。
...
- （3）[感觉这个比较好，全局唯一id去重] 比如你不是上面两个场景，那做的稍微复杂一点，你需要让生产者发送每条数据的时候，里面加一个全局唯一的id，类似订单id之类的东西，然后你这里消费到了之后，先根据这个id去比如redis里查一下，之前消费过吗？如果没有消费过，你就处理，然后这个id写redis。如果消费过了，那你就别处理了，保证别重复处理相同的消息即可。

还有比如基于数据库的唯一键来保证重复数据不会重复插入多条，我们之前线上系统就有这个问题，就是拿到数据的时候，每次重启可能会有重复，因为kafka消费者还没来得及提交offset，重复数据拿到了以后我们插入的时候，因为有唯一键约束了，所以重复数据只会插入报错，不会导致数据库中出现脏数据

`如何保证MQ的消费是幂等性的，需要结合具体的业务来看`

![](../../pic/2020-10-17/2020-10-17-23-28-10.png)


# 06、如何保证消息的可靠性传输（如何处理消息丢失的问题）？


如果说你这个是用mq来传递非常核心的消息，比如说计费，扣费的一些消息，因为我以前设计和研发过一个公司非常核心的广告平台，计费系统，计费系统是很重的一个业务，操作是很耗时的。所以说广告系统整体的架构里面，实际上是将计费做成异步化的，
然后中间就是加了一个MQ。

我们当时为了确保说这个MQ传递过程中绝对不会把计费消息给弄丢，花了很多的精力。广告主投放了一个广告，明明说好了，用户点击一次扣费1块钱。结果要是用户动不动点击了一次，扣费的时候搞的消息丢了，我们公司就会不断的少几块钱，几块钱，
积少成多，这个就对公司是一个很大的损失。
...
![](../../pic/2020-10-17/2020-10-17-23-34-13.png)
...
这不是一样么，大家都知道kafka会自动提交offset，那么只要关闭自动提交offset，在处理完之后自己手动提交offset，就可以保证数据不会丢。但是此时确实还是会重复消费，比如你刚处理完，还没提交offset，结果自己挂了，此时肯定会重复消费一次，
自己保证幂等性就好了。
...
![rabbitmq会出现顺序乱的问题](../../pic/2020-10-17/2020-10-17-23-39-51.png)

![rabbitmq使用一个queue存储保证顺序](../../pic/2020-10-17/2020-10-17-23-40-23.png)

（2）kafka：一个topic，一个partition，一个consumer，内部多线程，这不也明显乱了

![kafka乱序问题](../../pic/2020-10-17/2020-10-17-23-41-52.png)

![](../../pic/2020-10-17/2020-10-17-23-42-59.png)
...
# 08、如何解决消息队列的延时以及过期失效问题？消息队列满了以后该怎么处理？有几百万消息持续积压几小时，说说怎么解决？
...
![](../../pic/2020-10-17/2020-10-17-23-46-46.png)
...
# 09、如果让你写一个消息队列，该如何进行架构设计啊？说一下你的思路
